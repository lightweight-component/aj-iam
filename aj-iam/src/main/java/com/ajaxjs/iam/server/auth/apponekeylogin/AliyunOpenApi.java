    ...
    private static String calculateSignature(Map<String, String> params, String accessKeySecret) {
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            ...
            canonicalizedQueryString.append("&")
                    .append(SendAliyunSms.percentEncode(entry.getKey())).append("=")
                    .append(SendAliyunSms.percentEncode(entry.getValue()));
        }
        // 移除第一个 &
        if (!canonicalizedQueryString.isEmpty())
            canonicalizedQueryString.deleteCharAt(0);

        String stringToSign = "POST&" + SendAliyunSms.percentEncode("/") + "&" + SendAliyunSms.percentEncode(canonicalizedQueryString.toString());
