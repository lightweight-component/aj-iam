...
public class TestAvatar extends BaseTest {
    ...
    void testWriteAvatar() {
        ...
        try (FileInputStream inputStream = new FileInputStream(imageFile)) {
            UpdateResult update = Sql.instance().input("UPDATE user SET avatar_blob = ? WHERE id = 25", inputStream).update();
...
void testWriteAvatarWebP() throws IOException {
    ...
    @Test
    void testGetAvatar() {
        String avatar = Sql.instance().input("SELECT avatar_blob FROM user WHERE id = 25").queryOne(String.class);
